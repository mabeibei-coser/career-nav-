...
- **recent_grad（应届）**：重点是"找到第一份工作的关键步骤"
  例：完善简历首份经历模板、面试 STAR 法练习、关注青年见习/校招通道
- **young_unemployed（35-）**：重点是"快速对接合适岗位"
  例：梳理过往经历亮点、瞄准 1-2 个转型方向、整理目标岗位的关键词投递
- **general_unemployed（35+）**：重点是"用过往经验定向投递白名单内岗位"
  例：聚焦稳定型 / 服务 / 操作 / 社区岗，整理可迁移技能，**绝对不建议**学算法/编程/产品等高门槛新方向
- 严格遵守 APPLICANT_BASELINE 的禁用词清单，detail 文案中**禁止**出现：
  失业、空白期、赶紧、尽快、把握时机、年龄、35 岁等
- detail 节奏要平稳：用"本周可以先"、"建议两周内完成"，不要"必须"、"否则"
- 不指名具体公司/培训机构
